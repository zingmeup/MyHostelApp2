...
function userTokenExists(){
	require $_SERVER['DOCUMENT_ROOT']."/api/warden/cnxn/conxwarden.php";
...
function getUserBasics(){
	...
	if ($response['Message']!="Invalid Acess Token") {
		...
		$GLOBALS['result']['user_info']['room']=$response['Type'];
		return TRUE;
	}else{
		return FALSE;
	}
...
function getUserPersonal(){
	$url="http://www.cuims.in/CUServices/api/mobapp/GetEmployeeInfo?userid=".$GLOBALS['args']['user_id']."&AcessToken=".$GLOBALS['args']['access_token'];     
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $url);
	curl_setopt($ch, CURLOPT_HTTPHEADER, $GLOBALS['headers']);
	curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, $GLOBALS['timeout']['connection']); 
	...
	if ($response['Message']!="Invalid Acess Token") {
		...
		$GLOBALS['result']['user_info']['user_id']=$response['EmployeeCode'];
		$GLOBALS['result']['user_info']['name']=$response['Name'];
		return TRUE;
	}else{
		return FALSE;
		...
		if ($authentication['Message']=="Success") {
			...
			if ($authentication['UserType']=='E') {
				...
				if (!getUserBasics()) {
					...
					if ($GLOBALS['result']['user_info']['user_type']=="H") {
