...
function getCurrentDayout(){
	require $_SERVER['DOCUMENT_ROOT']."/warden/cnxn/conxwarden.php";
	$stmtgetCurrentDayout->execute();
	$stmtgetCurrentDayout->setFetchMode(PDO::FETCH_ASSOC);
	$result=$stmtgetCurrentDayout->fetchAll();
	$con=null;
...
function checkHeaders(){
	...
	if (isset($GLOBALS['headers']['reqty'])&&!empty($GLOBALS['headers']['reqty'])){
		if ($GLOBALS['headers']['reqty']=='current_dayout') {
			return true;
		}
	}
	return false;
