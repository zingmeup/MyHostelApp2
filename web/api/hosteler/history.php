...
function getLeaveHistory(){
	require $_SERVER['DOCUMENT_ROOT']."/api/hosteler/cnxn/conxhosteler.php";
	$stmtLeaveHistory->execute();
	$stmtLeaveHistory->setFetchMode(PDO::FETCH_ASSOC);
	$result=$stmtLeaveHistory->fetchAll();
	$con=null;
	$GLOBALS['result']['leave']['list_count']=count($result);
	if (count($result)>0) {
		$GLOBALS['result']['leave']['list']=$result;
	}
	return FALSE;
}
...
function getDayoutHistory(){
	...
	$con=null;
	$GLOBALS['result']['dayout']['list_count']=count($result);
	if (count($result)>0) {
		$GLOBALS['result']['dayout']['list']=$result;
	}
	return FALSE;
}
